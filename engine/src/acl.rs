...
pub struct Acl {
    ...
    v4: Vec<(u32, u32)>,
    v6: Vec<(u128, u128)>,
    /// Both families are fully covered: every client matches.
    all: bool,
}
...
impl Acl {
    ...
    pub fn parse(cidrs: &[String]) -> Result<Acl, String> {
        ...
        let v4 = merged(v4, |x| x.checked_add(1));
        let v6 = merged(v6, |x| x.checked_add(1));
        let all = v4 == [(0, u32::MAX)] && v6 == [(0, u128::MAX)];
        Ok(Acl { v4, v6, all })
    }

    /// Covers every IPv4 and IPv6 address (for example 0.0.0.0/0 and ::/0).
...
mod tests {
    ...
    fn overlapping_cidrs_merge_into_ranges() {
        ...
        assert!(all.allows_all() && all == Acl::any());
        assert!(!acl.allows_all());
        assert!(!Acl::parse(&["0.0.0.0/0".into()]).unwrap().allows_all());
        assert!(
            !Acl::parse(&[])
                .unwrap()
