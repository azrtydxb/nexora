# M1 Task 17: Blocklist subscriptions — fetcher, parser, normalised blobs, and the subscription acceptance test

Status: open
Created: 2026-09-13

## Description

Implement Task 17 ("Blocklist subscriptions — fetcher, parser, normalised blobs, and the subscription acceptance test") of milestone M1 exactly as specified in
`.procoder/plans/nexora-v1-m1.md`, following `docs/architecture.md`. Done means
the task's tests exist, fail before the change, pass after it, and the change
is committed.

## Acceptance criteria

- [ ] Every step of Task 17 in `.procoder/plans/nexora-v1-m1.md` is done as written (deviations recorded in the plan first)
- [ ] `TestBlocklistSubscription` passes in the dev pod (`scripts/dev-exec.sh`)
- [ ] `TestNormalizeAndCompressAreDeterministic` passes in the dev pod (`scripts/dev-exec.sh`)
- [ ] `TestParseFormats` passes in the dev pod (`scripts/dev-exec.sh`)
- [ ] procoder gate clean over the changed files; work committed

## Evidence

