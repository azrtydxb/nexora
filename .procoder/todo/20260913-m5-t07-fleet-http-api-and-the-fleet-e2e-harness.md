# M5 Task 7: Fleet HTTP API and the fleet e2e harness

Status: open
Created: 2026-09-13

## Description

Implement Task 7 ("Fleet HTTP API and the fleet e2e harness") of milestone M5 exactly as specified in
`.procoder/plans/nexora-v1-m5.md`, following `docs/architecture.md`. Done means
the task's tests exist, fail before the change, pass after it, and the change
is committed.

## Acceptance criteria

- [ ] Every step of Task 7 in `.procoder/plans/nexora-v1-m5.md` is done as written (deviations recorded in the plan first)
- [ ] `TestAuthRBACAuditOIDC` passes in the dev pod (`scripts/dev-exec.sh`)
- [ ] `TestFleetAPI` passes in the dev pod (`scripts/dev-exec.sh`)
- [ ] `TestFleetAPIHelpersMatchOpenAPI` passes in the dev pod (`scripts/dev-exec.sh`)
- [ ] `TestFleetPermissions` passes in the dev pod (`scripts/dev-exec.sh`)
- [ ] `TestGUICoverage` passes in the dev pod (`scripts/dev-exec.sh`)
- [ ] `TestMgmtStatelessHA` passes in the dev pod (`scripts/dev-exec.sh`)
- [ ] procoder gate clean over the changed files; work committed

## Evidence

