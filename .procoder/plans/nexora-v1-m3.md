...
- [x] Create `engine/src/recursor/dispatch_tests.rs`:
...
- [x] Run `scripts/dev-exec.sh cargo test --locked -p nexora-engine --lib recursor::dispatch_tests` — expect FAIL with "unresolved import `super::dispatch`".
- [x] Implement `dispatch.rs`:
...
- [x] Implement `RecursorState::new(state_dir)` (metrics, `Recursor::new`; in this task also `Validator::new(metrics.clone())` and `TrustAnchorStore::open(state_dir.map(|d| d.join("trust-anchors.json")))` as empty types with those constructors in `dnssec/validator.rs` and `dnssec/anchors.rs`, whose bodies Tasks 7–8 fill; `open(None)` keeps state in memory only).
- [x] Integrate into the engine:
...
- [x] Run `scripts/dev-exec.sh cargo test --locked -p nexora-engine --lib recursor::dispatch_tests` — expect PASS (4 tests).
- [x] Run the whole engine suite to prove the hot path is untouched: `scripts/dev-exec.sh make engine-test` — expect PASS including `cache_hit_path_does_not_allocate`, `policy_pipeline` and `server_pipeline` (forward mode is the default and forwards the client's bytes).
- [ ] Commit: `git add engine && git commit -m "feat(engine): resolution mode selection, forward zones and recursive miss path"` (the lead commits).

As built (Task 5, together with the integration steps of Tasks 7-10):

- `Ede` is defined once in `engine/src/recursor/mod.rs` (`Ede { code, text }` with `Ede::new`); `dnssec/mod.rs`, `rpz/apply.rs` and `dispatch.rs` re-export it.
- `RecursorState` also holds `rpz: rpz::RpzState` and has `sync(&Runtime)` (anchors merged, `validator.clear()` when the trust points changed, RPZ configuration handed to the manager). `TrustPoint`/`TrustPoints` derive `PartialEq` for that comparison. `spawn_background(shared) -> io::Result<JoinHandle>` and the anchor refresh loop live in `recursor/mod.rs` (not `anchors.rs`, which would otherwise depend on `server`).
- `ResolutionRuntime` has no `#[derive(Default)]` (`RecursionParams` has none); `Default` is implemented as forward mode with IANA hints. It gains `aggressive_nsec: bool` (from `RecursionConfig.aggressive_nsec`), `rpz: Vec<RpzZoneConfig>` (Task 10) and `validates(&Route)` / `validates_any_route()`. `config_key` hashes the sections in the order resolution_mode, recursion, forward_zones, dnssec, rpz_zones, then `dnssec_validate_forwarded`.
- `MissQuery::from_view` decodes the name with `Name::from_bytes`. `resolve_miss` runs the query-phase RPZ `Apply` (local-data CNAME chase), resolves on the route of the name, validates (Task 7), then runs the response-phase RPZ check (Task 9). Forward-zone exchanges use a 2000 ms per-server timeout. A hit in a DISABLED zone logs `rpz_action = RPZ_DISABLED (7)` and stays cacheable.
- `server::resolve_miss`: misses carrying an RPZ decision call `dispatch::resolve_miss` without in-flight coalescing, skip the cache lookup and insert, and still run the CNAME-cloaking check; `leader_answer` gained an `insert: bool` (false when the runtime or the RPZ set changed during resolution). `rpz_reply` also takes the `QueryView`.
- `QueryRecord` names for OTLP are `querylog::{ROUTE_NAMES, DNSSEC_NAMES, RPZ_NAMES, name_at}`.
- The trust-anchor refresh runs only while `ResolutionRuntime::validates_any_route()` (validation on and recursive mode, `dnssec_validate_forwarded`, or a validating forward zone), so engines whose routes never validate send no DNSKEY queries to their upstreams.
- Tests: the plan's 4 plus `forward_mode_validates_when_enabled` (Task 7, a signed in-memory forwarder: secure -> AD, broken signature -> SERVFAIL EDE 6 and not failed, `validate_forwarded = false` -> the upstream's bytes unchanged) and `rpz_response_ip_trigger_and_local_data_cname_chase` in `dispatch_tests.rs` (6 tests); `engine/tests/rpz_pipeline.rs` (RPZ file zone on running workers: QNAME NXDOMAIN with EDE 15 on the wire, local data EDE 4, response-IP NODATA, never cached, hit counts); `cache.rs` `ad_is_cleared_for_clients_without_do_or_ad`; `edns.rs` EDE option bytes; `cache_hit_path_does_not_allocate` now measures forward mode, recursive mode with validation, and loaded RPZ query triggers.
...
- [x] Wire into `dispatch::resolve_miss` (done with Task 5; `forward_mode_validates_when_enabled` is in `dispatch_tests.rs`):
...
- Tests: the plan's 8, plus `forward_mode_validates_through_the_forwarder` (a forwarder over the in-memory hierarchy answers `validation_query` wire queries; secure name -> `Secure`, broken signature -> `Bogus` EDE 6, all DS/DNSKEY fetched through it), `unreachable_authority_is_indeterminate` and `wildcard_expansion_needs_a_next_closer_proof` (11 tests). The dispatch-level `forward_mode_validates_when_enabled` case and the `dispatch.rs` steps were built with Task 5 (see its As-built notes); the validating route uses `validates(route) && !checking_disabled`, a separate `WorkBudget` for chain-of-trust fetches, and caps record TTLs with `ttl_cap`.
...
- `refresh_loop` (private, in `recursor/mod.rs`), `spawn_background`, `RecursorState::sync`, and the `control.rs`/`main.rs`/`telemetry/metrics.rs` steps were built with Task 5; the loop skips engines where no route validates.
...
- [x] Integration (after Task 5) — wire into the query path (built with Task 5):
...
- [x] Integration (after Task 5) — complete the RPZ wiring (built with Task 5; `rpz_reply` also takes the `QueryView`, a DISABLED-zone hit logs `rpz_action` 7, and RPZ misses still get the CNAME-cloaking check):
...
- [x] Integration (after Task 5, built) — `mod.rs`: `RecursorState` gains `pub rpz: rpz::RpzState` (`RpzState::new(state_dir)`, same `state_dir`); `sync` also calls `self.rpz.manager.apply_config(&self.rpz, &rt.resolution.rpz)`; `spawn_background` also runs `shared.recursor.rpz.manager.run(&shared.recursor.rpz, &shared.runtime)` on the `nexora-recursor` `LocalSet`.
- [x] Integration (after Task 5, built) — `control.rs` `session`: replace the Task 1 no-op arm with `Some(ServerMsg::RpzTsigKeys(keys)) => shared.recursor.rpz.set_tsig_keys(keys),` (no log line carries the message); `Metrics::stats` sets `rpz_zones: recursor.rpz.manager.status()` and `render` calls `recursor.rpz.manager.register_metrics(&mut reg)`.
...
As built (reconciled with the M2 kw deployment: HTTPS-only API, `loadKwEnv`/`kwLogin` helpers, DaemonSet engines):

Files:

- `e2e/kw_smoke_test.go` — `TestKwSmoke` ends with `kwSmokeM3`, subtests `dnssec-forwarded`, `recursion`, `trust-anchors`, `rpz`, `m3-metrics` (same environment as M1/M2, plus optional `NEXORA_KW_ENGINE_METRICS_URL`, default `http://nexora-engine-metrics.nexora.svc.cluster.local:9153/metrics`) (modify)
- `deploy/kw/bootstrap.sh` — idempotently keeps resolution mode `forward`, DNSSEC `validation` and `validate_forwarded` on, and creates/uploads the RPZ file zone `rpz.kw.nexora.` (`example.net CNAME .`) (modify)
- `deploy/kw/mgmt.yaml` — env `NEXORA_KEK_FILE=/etc/nexora/kek/kek`, volume `kek` from Secret `nexora-kek` (`defaultMode: 0440`) mounted read-only at `/etc/nexora/kek` (modify; the layout M4 Task 16 specifies, pulled forward because RPZ TSIG secrets need the KEK)
- `scripts/kw-deploy.sh` — creates `nexora-kek` once: `openssl rand -base64 32 | k create secret generic nexora-kek --from-file=kek=/dev/stdin` (modify)
- `deploy/kw/README.md` — `nexora-kek` under "Secrets", M3 subtests and manual checks, "Resolution settings", M3 limits (modify)

kw's network transparently redirects all outbound UDP/TCP 53 to a resolver (a non-recursive query to `198.41.0.4` or even `192.0.2.1` gets a recursive answer), so recursion from the real root cannot work on kw. kw therefore runs forward mode with forwarded-answer validation (which works through the redirect: it passes DNSSEC records). `recursion` probes a.root-servers.net with RD=0; on a recursive answer it skips with that reason, otherwise it switches to recursive mode, expects an AD answer for `www.isc.org` and bogus `sigfail.verteiltesysteme.net`, and restores the mode.

Subtests: `dnssec-forwarded` asserts mode `forward`, `validation` and `validate_forwarded` true, AD for `www.iana.org`/`cloudflare.com`, and `dnssec-failed.org` SERVFAIL with DO but an answer with CD (`kwWantBogus`); `trust-anchors` wants at least `NEXORA_KW_ENGINES` engines with root anchor state `valid` or `configured` in `/dnssec/status`; `rpz` wants `example.net` NXDOMAIN and `example.org` NOERROR, then adds zone `rpz.kwsmoke.nexora.` blocking `example.org` (NXDOMAIN after `kwWaitApplied`), deletes it and wants NOERROR again; `m3-metrics` wants `nexora_resolutions`, `nexora_dnssec_validations`, `nexora_rpz_zone_serial{zone="rpz.kw.nexora."}` and `nexora_dnssec_trust_anchor_keys` from the engine metrics Service.

- [x] Build images from a clean worktree of the M3 HEAD: `git worktree add /tmp/nexora-m3-build HEAD`; `scripts/build-image.sh -f deploy/docker/{engine,mgmt}.Dockerfile -n nexora-{engine,mgmt} -t sha-<7> /tmp/nexora-m3-build`; remove the worktree.
- [x] Deploy: `scripts/kw-deploy.sh --skip-build --tag sha-<7>` — both rollouts complete, bootstrap prints `RPZ zone rpz.kw.nexora. created`/`uploaded`; a second `deploy/kw/bootstrap.sh` prints nothing (idempotent).
- [x] Run `TestKwSmoke` from the dev pod with the README environment — PASS (M1/M2 subtests, `dnssec-forwarded`, `trust-anchors`, `rpz`, `m3-metrics`; `recursion` SKIP on kw).
- [x] Manual: `delv @192.168.10.136 dnssec-failed.org` — resolution failed (SERVFAIL, EDE 9); `dig @192.168.10.136 +dnssec cloudflare.com` — `ad` flag set.
