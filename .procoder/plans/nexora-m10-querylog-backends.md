...
- **Loki partitions:** `LokiPartitionRegexes(field)` returns 37 regexes; `<c>` is
  `"0123456789abcdefghijklmnopqrstuvwxyz"[i]` and every regex is case-insensitive with `s`.
  - **Names** (first character after one optional leading `www.`, so `.com.` traffic spreads):
    partition `i` is `(?is)^(?:www\.)?<c>.*$`, except `w`, which must exclude `www.x` and, without
    lookahead in RE2, is `(?is)^(?:www\.w.*|w(?:[^w].*|w(?:[^w].*|w(?:[^.].*)?)?)?)$`. Partition 36
    ("none") is `(?is)^(?:|www\.(?:[^0-9a-z].*)?|[^0-9a-z].*)$` (empty, `www.` alone, the root `.`,
    `_` labels).
  - **Clients and categories** (last character, ignoring one trailing dot): partition `i` is
    `(?is)^.*<c>\.?$` and partition 36 is `(?is)^(?:\.?|.*[^0-9a-z.]|.*[^0-9a-z]\.)$`.
  - The first draft partitioned every field by last character; the lead changed names to the first
    character because nearly all names end in the TLD's last letter.
...
- **Toolbox image tag:** the next unused `toolbox-N` in `192.168.10.131/azrtydxb/nexora-dev`, found
  with `curl -sk https://192.168.10.131/v2/azrtydxb/nexora-dev/tags/list` (`toolbox-2`). It is set
  in `deploy/dev/dev-pod.yaml`; Task 3 switched only `toolbox-m10`, and the lead rolls it out to
  `toolbox` and any other `toolbox-*` deployment in `nexora-dev`.
...
func PollVisible(b querylog.Backend, base time.Time) func(t testing.TB, want int) // 250 ms polls, 60 s
```
...
- [x] Create `mgmt/internal/querylog/conformance_test.go`:
  ...
  ```
  `Run` takes `testing.TB`, and so do the `Harness` functions. Its subtests run through a helper
  `sub(tb, name, fn)`: it calls `t.Run` when `tb` is a `*testing.T`, and otherwise calls `fn(tb)`
  directly, so the recording TB sees every failure. `lossy` embeds `*querylog.Builtin` (not
  `querylog.Backend`) so it keeps `Top`: a wrapper without `Top` would fail the suite at the `Topper`
  assertion and pass the test even if the paging checks caught nothing. A mutation check (lossy made
  lossless) makes `TestConformanceDetectsDivergence` fail.
- [x] Run
      ...
      and expect FAIL: `no required module provides package github.com/piwi3910/nexora/mgmt/internal/querylog/querylogtest`.
...
- [x] Create `conformance.go`. `Run(tb, run, base, h)`:
  ...
  3. For each named case, build a `querylog.Query` with `From, To = Window(base)`. Cases whose name
     matching is not what they test also set `Name: <run>` (every dataset name contains the run;
     `"-<run>."` would miss `x%y_<run>.test.` and `www.you-<run>.tube.test.`).
  ...
  5. Also assert the fixed expectations:
     - `partial-name`:
       ...
       - `x%y_`+run → 1 (`%` is literal; as a wildcard it would also match `x*y-`);
       - `absent-`+run → 0.
     - `multi-value`:
       - `QTypes {A, AAAA}` → every in-window record except the MX one, A/AAAA only, AAAA included;
       - `RCodes {NXDOMAIN}` + `QTypes {A}` → `you-` and `rpz-` (2);
       - `Caches {hit}` → 1;
       - `Client 10.0.0.2` → `you-`; `Categories {gambling, ads-tracking}` → 4;
         `ListIDs {l-custom, allow-g1}` → `ads-` and `www.you-`; `EngineIDs {e2}` → only e2 records.
     - `reason-fields`: `Sources {allowlist}` → 1 record with `Rule == "tube.test"`,
       ...
       `RPZAction == "nxdomain"`), `acl` (`ACLRefused == "authoritative"`), `rewrite` (rule), `race`
       (`UpstreamsRaced == 3`, `DurationUS == 1234`, `Transport == "tcp"`) and every default field of
       `edge-from` (including `RPZAction == ""` for `nexora.rpz=none` and `Time == From`).
     ...
     - `time-range-inclusive`: first (positive path) the window widened by 1 s on each side holds
       `before` and `after`; then the full window includes `edge-from` and `edge-to` and excludes
       `before` and `after`.
     ...
     - `top-name`: Top `Field name, Limit 4` → `top-a:4`, then `tie:3`, `top-b:3` and `top-c:3` in key
       order (the three `tie-<run>` records count 3 too).
     - `top-ties-key-ascending`: `Limit 2` → `top-a:4, tie:3` (not `top-b` or `top-c`).
     - `top-blocked`: `Filters {blocked}`, `Field name`, `Limit 10` → `top-b:3`, then `ads`, `casino`
       and `renamed` counted 1.
     - `top-client`: `Field client, Limit 1` → `10.0.0.1:13`, equal to the reference.
     - `top-category-skips-empty`: `Field category` → exactly `ads-tracking:3, gambling:1`, no empty
       key.
     - `top-time-range-inclusive`: first (positive path) the window widened by 1 s holds `before` and
       `after`; then `From`/`To` set to the offsets of `edge-from`/`edge-to` with `Field name`
       includes both edges and excludes `before` and `after`.

     Every Top case compares with `ref.Top` for equality (`slices.Equal`) before the fixed
     ...
     a unique index.
- [x] Add to `conformance.go`: `OTLPExport` dials `grpcAddr` with `grpc.NewClient(addr, grpc.WithTransportCredentials(insecure.NewCredentials()))` and calls `collogspb.NewLogsServiceClient(conn).Export` once per batch in order, failing the test on error. `PollVisible(b, base)` returns a closure that polls `b.Search(Query{From, To: Window(base), Limit: 1000})` every 250 ms for up to 60 s until `len(Records) >= want`, then fails with the last count.
...
- [x] Add to `mgmt/internal/config/config_test.go`:
  ...
  ```
  The imports gain `maps` and `strings` (`time` was already imported); `load` uses the file's `env`
  helper. The existing `TestLoadValidation` "bad backend" case used `loki`, which becomes valid; it
  now uses `elastic`, so it still tests an unknown backend.
...
- [x] Implement in `config.go`:
  ...
  - `NEXORA_LOKI_LOOKBACK` is parsed and range-checked whatever the backend (like the other duration
    settings).
- [x] Run the command from step 2 and expect PASS. Then run
...
- [x] Edit `deploy/dev/Dockerfile`:
  ...
  - Append to the download `RUN`:
    ...
    ```
  - The `v26.8.4.11-lts` tag exists; the archive holds
    `clickhouse-common-static-26.8.4.11/usr/bin/clickhouse`.
- [x] Find the next unused tag:
      ...
      (it was `toolbox-1`, so the new tag is `toolbox-2`). Then build and push with kw BuildKit:
      `scripts/build-image.sh -f Dockerfile -n nexora-dev -t toolbox-2 deploy/dev` (platform
      `linux/arm64`, output `192.168.10.131:5000/azrtydxb/nexora-dev:toolbox-2`). Expect the push to
      succeed.
- [x] Set `image: 192.168.10.131/azrtydxb/nexora-dev:toolbox-2` in `deploy/dev/dev-pod.yaml`.
      During the parallel M10 build only the M10 dev pod switched:
      `kubectl --context kw -n nexora-dev set image deploy/toolbox-m10 toolbox=192.168.10.131/azrtydxb/nexora-dev:toolbox-2`
      and `kubectl --context kw -n nexora-dev rollout status deploy/toolbox-m10 --timeout=10m`. The
      lead applies `deploy/dev/dev-pod.yaml` (`kubectl --context kw apply -f deploy/dev/dev-pod.yaml`)
      and sets the image of the other `toolbox-*` deployments when no other work runs in them.
...
- [x] Create `e2e/harness/otelcol_test.go`:
  ...
  (0.160.0 accepts the `log.` prefixed form under `context: log`; `validate` does parse OTTL: a
  `log.bogus` path fails it.)
...
- `mgmt/internal/querylog/e2e/clickhouse_conformance_test.go`: created. `TestQueryLogConformanceClickHouse`
  (in Task 7's package: the top-level `e2e` package cannot import `mgmt/internal/...`).
...
- [x] Create `mgmt/internal/querylog/clickhouse_test.go` with these tests:
  - **`TestClickHouseQueryIsParameterised`:**
    ...
    - Assert:
      ...
      - `param_name` is, as a raw string, `%a\\%b\\_c\\\\'); drop%`: LIKE escaping (`\`, `%`, `_`), then
        the escaped-format escaping ClickHouse applies to every scalar `String` parameter (it reads
        `\\` as `\`; verified against 26.8.4.11: the singly escaped pattern matches nothing);
  ...
  - **`TestClickHouseArrayParamEscapesQuotes`:** `ListIDs {a'b\c}` sends `param_list_ids` `['a\'b\\c']`
    (array elements are quoted literals, one escape level) and `Client "x<tab>y"` sends
    `param_client` `x\ty`.
  - **`TestClickHouseCursorKeyset`:**
    ...
    - The cursors `"x"`, a base64 `[1]`, a base64 `["a","b"]` and a base64 `[1,"not-a-uuid"]` each give `ErrInvalidCursor`
  ...
  - **`TestClickHouseErrors`:**
    ...
    - A 403 with code 516 names `NEXORA_CLICKHOUSE_PASSWORD_FILE`; a 404 with code 81 also names the SQL
      file, and a 400 carries ClickHouse's message. Top maps the same errors.
    - A table name that is not a plain identifier and an unreadable password file fail `NewClickHouse`.
...
- [x] Implement `mgmt/internal/querylog/clickhouse.go`:
  ...
  - **`do(ctx, sql string, params map[string]string, out func(dec *json.Decoder) error)`:**
    - POSTs the SQL (ending in `FORMAT JSONEachRow`, so a read-only profile needs no settings change)
      as the body to `URL/?database=<db>&param_<k>=<v>...`, with `X-ClickHouse-User`, and
      `X-ClickHouse-Key` when a password is set.
  ...
  - **`escapeClickHouseParam(v)`** escapes `\`, tab, newline and carriage return for scalar
    `String` parameters (`Client`, `Name`).
  ...
  - **Search** selects `toString(toUnixTimestamp64Nano(Timestamp)) AS t, toString(RowID) AS rowid`
    ...
    at limit+1. `NextCursor` is set from the last kept row when limit+1 rows arrived. A cursor whose
    row id is not a lowercase UUID is `ErrInvalidCursor` without a request.
  - **Top** uses the `key` mapping `name→Name`, `client→Client`, `category→Category`, and
    `count() AS c` (a string alias would sort `c` as text). 64-bit integers are decoded whether
    ClickHouse quotes them (`"3"`) or not. `TopQuery.Limit <= 0` uses 10.
...
- [x] Create `e2e/harness/clickhouse.go`. `StartClickHouse`:
  ...
  6. The grants live in `users.d/nexora.xml` as `<grants><query>GRANT INSERT ON nexora.querylog</query></grants>`
     (writer) and `GRANT SELECT` (reader): users defined in XML are read-only storage, so a SQL
     `GRANT ... TO nexora_writer` fails. Verified by hand: the reader's INSERT and the writer's
     SELECT get `Not enough privileges` (code 497).
- [x] Create `mgmt/internal/querylog/e2e/clickhouse_conformance_test.go` (package `e2e`, next to
      Task 7's `opensearch_conformance_test.go`, whose `requireE2E` skips outside `make e2e`):
  ```go
...
- [x] Run the step-2 command and expect PASS. Then run
      `scripts/dev-exec.sh 'make e2e-build && NEXORA_E2E_BIN_DIR=/work/nexora/bin go test ./mgmt/internal/querylog/e2e -run TestQueryLogConformanceClickHouse -count=1 -v -timeout 20m'`
...
- [x] Add the comment
      ...
      above the column list (`clickHouseRecordColumns`) in `clickhouse.go`.
- [x] Report the paths. Commit message: `querylog: clickhouse backend`.
...
- `mgmt/internal/querylog/loki_logql.go`: created. Quoting, filter stages, partitions, tiebreak key
  and record decoding.
- `mgmt/internal/querylog/loki_test.go`: created. Unit tests.
- `e2e/harness/loki.go`: created. `StartLoki`.
- `mgmt/internal/querylog/e2e/loki_conformance_test.go`: created. `TestQueryLogConformanceLoki`. It
  lives in Task 7's package `mgmt/internal/querylog/e2e` (the top-level `e2e` package may not import
  `mgmt/internal/...`) and skips through `requireE2E` unless `NEXORA_E2E_BIN_DIR` is set.
...
func NewLoki(cfg config.LokiConfig) (*Loki, error) // empty Selector and zero Lookback use the config defaults
...
func LokiPartitionRegexes(field TopField) []string // 37 regexes; index 36 is "none"
...
Loki 3.6.7 facts found while building this task (they change the original plan text):

- The series-limit 400 reads `maximum number of series (N) reached for a single query`. The adapter
  accepts it and the older `maximum of series`.
- LogQL range selectors reject `ns` (`unknown unit "ns" in duration`) and accept `ms`. Ranges are
  left-open, `(time − range, time]`, while `time` and `offset` keep nanoseconds. No combination of
  whole-millisecond ranges covers `[From, To]` exactly (its length is a whole number of ms plus 1 ns),
  so Top uses `rangeMS = floor((To−From)/1ms) + 1` evaluated at `time = From − 1ns + rangeMS`: the
  window starts exactly at From and ends less than 1 ms after To (nothing when To is now). A `debt:`
  comment marks it.
- The series limit applies to the `sum by` output (inside `topk` too), not to the raw
  `count_over_time` series.
...
- [x] Create `mgmt/internal/querylog/loki_test.go`. A fake Loki (`httptest`) records each request's
  ...
  - **`TestLokiLogQLQuotesValues`:**
    - Search with `Name "A(b).c."`, `QTypes {A, "AA|AA"}`, `PolicyGroups {global, g1}`,
      `Filters {blocked}`, `Client "10.0.0.1"`, `From`/`To` set, tenant `t1` and basic auth.
    - Assert the query contains:
      - `{service_name="nexora-engine"}`;
      - `dns_question_name=~"(?i)^.*a\\(b\\)\\.c.*$"` (the fragment is lowercased, the trailing dot
        removed and the metacharacters quoted, then Go-quoted);
    ...
    - Assert `start == From.UnixNano()`, `end == To.UnixNano()+1`, `direction == backward`,
      `limit == 11`, the headers `X-Scope-OrgID: t1` and `X-Loki-Response-Encoding-Flags: categorize-labels`,
      and basic auth with the password file's content.
    - Assert the decoded record (every field, `Filter` from `nexora_filter_result`, `nexora_rpz=none`
      as `""`, integers parsed).
    - Assert that a user value `x" or client_address=~".*\n` as Name and Client appears only as the
      two quoted literals.
    - Assert that a zero From gives `start == To − 168h` and the bare selector.
  ...
  - **`TestLokiTopTwoStepTies`:**
    ...
    - For a 60 s window the first query contains `topk(2, sum by (dns_question_name) (count_over_time(`,
      `dns_question_name!=""` and `[60001ms]`, and `time == From.UnixNano() − 1 + 60001ms`.
    - The second query ends in `) >= 3`.
    - An empty step-1 vector gives `[]` and no second request.
  - **`TestLokiTopPartitionFallback`:**
    - The fake answers HTTP 400 `maximum number of series (500) reached for a single query` to any
      query without a `dns_question_name=~"(?is)^` partition matcher.
    - For partitioned queries it computes the answers from an in-memory map of 200 names with
      counts `1 + i%7` (first characters over all 37 partitions, half behind `www.`). It tracks the maximum number of
      in-flight requests.
    - Top `Limit 10` equals the expectation computed directly from the map: count desc, key asc,
      first 10.
    - The maximum in-flight count is between 2 and 6, and exactly 37 step-1 partition queries were
      sent.
  ...
  - **`TestLokiPartitionsCoverEveryKey`:**
    - Compile `LokiPartitionRegexes(field)` with Go `regexp` for name, client and category.
    - Names, positive path first: sample names land in exactly their expected partition, for example
      `www.google.com.` in `g`, `1.1.1.1.in-addr.arpa.` in `1`, `xn--bcher-kva.example.` and
      `www.xn--…` in `x`, `wwwx.com.`, `www.w.com.` and `www` in `w`, and `.`, `""`, `www.`, `www..`,
      `_dmarc.example.com.` and `www._sip.example.` in none.
    - For 10,000 keys from `math/rand/v2` with a fixed seed (lengths 0–20 over `[0-9a-zA-Z.:\-_é]`,
      a third prefixed with `www.`, `WWW.`, `ww` or `www`), assert exactly one name regex matches.
    - Clients and categories: `10.0.0.13` is in partition 3, and the random keys plus `""`, `"."`,
      `"a."`, `"A"`, `"::1"`, `"fe80::a"`, `"x-"`, `"é"`, `"a.."`, `".."` and `"Z."` match exactly one
      regex of each.
  - **`TestLokiErrors`:**
    - The positive path first: a 200 success.
    - A 503 and a 429 each give `ErrBackendUnavailable`, and so does a closed server.
    - A 400 `parse error at line 1` is a plain error containing `parse error`.
    - The cursors `"x"`, base64 `[1]`, `["a","b"]`, `["1"]` and a ClickHouse-shaped
      `[1757844000123456789,"0b0c"]` give `ErrInvalidCursor` with no request.
    - Top `Field "bogus"` is an error; an unreadable password file names `NEXORA_LOKI_PASSWORD_FILE`.
...
- [x] Implement `loki_logql.go`:
  ...
  - Constants map Record fields to structured metadata names (the spec Data list), and
    `lokiTopLabels` maps the Top fields.
  - **`lokiStages(q Query) string`** builds ` | label="value"` for `Client` and
    ` | label=~<quoted regex>` for `Name` and each slice. `Filters` becomes
    ` | (nexora_filter=~R or nexora_filter_result=~R)`, and `PolicyGroups` maps `global` to `""`.
  - **`LokiPartitionRegexes(field)`** returns the regexes from the Architecture decision (names by
    first character after `www.`, clients and categories by last character).
  - **`lokiKey(line string, md map[string]string) string`** follows the Architecture decision.
  - **`recordFromLoki(ts int64, md map[string]string) Record`** (the line only repeats metadata)
    takes `Filter` from `nexora_filter`, else `nexora_filter_result`. `RPZAction` maps `none` to
    `""`, and `UpstreamsRaced` and `DurationUS` go through `strconv.ParseInt` with 0 on error.
- [x] Implement `loki.go`:
  ...
  - **`get(ctx, path, params, resultType) (json.RawMessage, error)`** sends basic auth when a
    username is set, `X-Scope-OrgID` when a tenant is set, and
    `X-Loki-Response-Encoding-Flags: categorize-labels`, and returns `data.result`.
    - A transport error, a status ≥ 500 or a 429 gives `ErrBackendUnavailable`.
    - A 400 whose body contains `maximum number of series` or `maximum of series` gives the
      unexported `errLokiSeriesLimit`.
  ...
  - **Search** runs inside `context.WithTimeout(ctx, lokiSearchTimeout)`:
    1. Decode the cursor (`[string ns, string key]`, where the ns string parses as int64), else
       `ErrInvalidCursor`.
    2. Set `start` to From (To − lookback when zero) and `end` to `To+1` (with To zero meaning now),
       or to `cursorNs+1`.
    3. Loop with `n := limit + 1`. Query `/loki/api/v1/query_range` with `limit=n` and
       ...
       outside `[start, end)` or with `ns == cursorNs && key <= cursorKey`.
    ...
    5. Sort by ns desc, then key asc. If the kept entries number fewer than `limit+1` and the
       response was full, set `n = min(2n, 5000)` and repeat. When `n` is already 5000 and nothing is
       kept, return
       ...
       When `n` is 5000 and some entries are kept, they form a short page with a cursor.
    6. The page is the first `limit`. `NextCursor` is `[ns, key]` of the last kept entry when a
       `limit+1`-th exists.
  - **Top** runs inside `context.WithTimeout(ctx, lokiTopTimeout)`; `Limit <= 0` gives `[]` (as the
    builtin):
    - `metric(partition)` gives
      `sum by (L) (count_over_time(<selector><stages> | L!=""<partition stage> [<rangeMS>ms]))`,
      evaluated at `time = From − 1ns + rangeMS` (see the Loki facts above).
    - Step 1 runs `topk(k, metric)` on `/loki/api/v1/query`, and `c` is the minimum value of the
      result. An empty result gives `[]` without step 2.
    - Step 2 runs `metric >= c`.
    - On `errLokiSeriesLimit` from either step, run step 1 for each of the 37 partitions (the extra
      stage `| L=~"<partition regex>"`) through a semaphore of 6, take the k-th largest count across
      all partition results as `c`, then run step 2 in each partition whose step-1 maximum reaches
      `c`, and merge.
...
- [x] Create `e2e/harness/loki.go`. `StartLoki(o)`:
  ...
  2. Write a config with:
     - `auth_enabled: false` and `analytics.reporting_enabled: false`;
     - `server.http_listen_port` and `grpc_listen_port` from `FreePort`, with both listen addresses
       `127.0.0.1`;
  ...
  3. Start `loki -config.file=<file>` and poll `GET /ready` until the body is `ready` within 60 s,
     retrying on other ports when loki exits with `address already in use`.
- [x] Create `mgmt/internal/querylog/e2e/loki_conformance_test.go`:
  - `TestQueryLogConformanceLoki` calls `requireE2E`, starts `StartLoki(LokiOptions{})` and
  ...
  - The subtest `t.Run("top-partitioned", ...)` starts a second env with `MaxQuerySeries: 5` and
    ingests the same dataset with a new run id into it and a builtin reference. It first asserts that
    a raw GET to `/loki/api/v1/query` with
    `sum by (dns_question_name) (count_over_time({service_name="nexora-engine"}[10m]))` returns 400
    containing `maximum number of series` (the positive precondition). It then asserts that
    `backend.Top` for `Field name, Limit 3` equals the builtin reference's `Top`. By first character
    the 20 in-window names spread over partitions of at most 5 (`t`: `tie` and the four `top-*`).
- [x] Run the step-2 command and expect PASS. Then run
      `scripts/dev-exec.sh 'make e2e-build && NEXORA_E2E_BIN_DIR=/work/nexora/bin go test ./mgmt/internal/querylog/e2e -run TestQueryLogConformanceLoki -count=1 -v -timeout 25m'`
      and expect PASS for every subtest and `top-partitioned`. Loki rejects `[<n>ns]` ranges, so Top
      uses `ms` as described above; `top-time-range-inclusive` passes.
- [x] Add the comment
      ...
      to the doc comment of `recordFromLoki`.
...
- `mgmt/internal/querylog/e2e/opensearch_conformance_test.go`: created. `TestQueryLogConformanceOpenSearch`.
  It lives under `mgmt/` because the top-level `e2e` package may not import `mgmt/internal/...` (Go's
  internal-package rule: `use of internal package ... not allowed`). It skips unless
  `NEXORA_E2E_BIN_DIR` is set, so `make mgmt-test` and CI unit jobs skip it.
- `Makefile`: the `e2e` target also runs `./mgmt/internal/querylog/e2e/...`.
- `mgmt/internal/querylog/opensearch.go`: only if the suite finds a divergence (none found).
- `mgmt/internal/querylog/opensearch_test.go`: a unit test for any fix (none needed).

Interfaces: consumes `querylogtest` (Task 1) and `OtelcolConfig.OpenSearchIndex` (Task 4). Produces
the package `mgmt/internal/querylog/e2e` and its `requireE2E(t)` skip helper, where Tasks 5 and 6 put
their conformance tests for the same internal-package reason.

- [x] Create `mgmt/internal/querylog/e2e/opensearch_conformance_test.go` (package `e2e`):
  ```go
  ...
  func TestQueryLogConformanceOpenSearch(t *testing.T) {
    ...
    t.Cleanup(func() {
        // The collector appends a date to the index name; run is unique, so the pattern matches
        // only this run's indices in the shared OpenSearch.
...
- [x] Run
      `scripts/dev-exec.sh 'make e2e-build && NEXORA_E2E_BIN_DIR=/work/nexora/bin go test ./mgmt/internal/querylog/e2e -run TestQueryLogConformanceOpenSearch -count=1 -v -timeout 20m'`.
      ...
      Result: PASS on the first run, no divergence, `opensearch.go` unchanged.
...
- [x] In `mgmt/api/openapi.yaml`, set the query-log page `backend` property to
      ...
      clients keep working). Regenerate `gen.go` in the dev pod with
      `go run github.com/oapi-codegen/oapi-codegen/v2/cmd/oapi-codegen@v2.8.0` (a copy of
      `oapi-codegen.yaml` whose `output` is a private path, copied back) and `schema.d.ts` with the pod's
      `openapi-typescript` (the `gen:api` script) into a private path, keeping the committed formatting.
      Expect only the new `Backend` doc comment (plus the embedded spec in `gen.go`).
- [x] Create `e2e/harness/backends_test.go`:
  ...
  ```
  `Env.T` is a `*testing.T`, so a recording `testing.TB` cannot stand in for it. `captureFatal` (a local
  helper in this file) re-executes the test binary with `-test.run=^<test>$` and
  `NEXORA_HARNESS_CAPTURE_FATAL=<test>` in the current environment (so the `t.Setenv` PATH applies).
  The child dispatches to `captureFatal` before starting anything, runs the function with a fresh
  `Env` and fails; the parent requires the child to fail and returns its output.
- [x] Create `e2e/querylog_backends_helper_test.go`. `queryLogMgmtOptions(t, env, backend)` returns
  ...
  - **opensearch:** the existing collector and URL lines, moved here, on a private index
    `nexora-e2e-<unique>` (collector `OpenSearchIndex`, management `NEXORA_OPENSEARCH_INDEX=<index>*`
    through `ExtraEnv`). The shared OpenSearch also carries live kw traffic: on the shared index
    `dashboard-top` never saw a 5-count name in the top 50, and fixed names (`www.custom.attr.test`,
    `TUBE.TEST`) matched earlier runs' records. The helper creates the day's index (and the next
    hour's day) with an `@timestamp` `date` mapping before the collector starts, because a search
    racing the exporter's auto-creation fails ("all shards failed", or "No mapping found for
    [@timestamp]" on an empty index). Cleanup stops the collector, then deletes `<index>*`.
...
- [x] Create `deploy/deploytest/helm_querylog_test.go`:
  ...
  ```
  `common` carries `database.mode=external` like `TestHelmTemplate`'s minimal renders (the default
  `cnpg` mode fails without the CNPG API). The committed test also asserts the Secret items
  (`key → password`), the read-only mount at `/etc/nexora/querylog`, `NEXORA_LOKI_USERNAME` rendered
  empty, and, as the positive path before the "no password file" assertion, a Loki render with
  `passwordSecret.name` that has `NEXORA_LOKI_PASSWORD_FILE` and the volume.
...
- [x] Implement:
  ...
  - **`mgmt-deployment.yaml`:**
    ...
    - The volume `querylog-password` (secret `passwordSecret.name`, items `key → password`) is
      mounted read-only at `/etc/nexora/querylog`, only when the selected backend's
      `passwordSecret.name` is set; builtin and opensearch renders (kw) are unchanged.
...
- `deploy/kw/clickhouse.yaml`: created. ConfigMap `clickhouse-config` (keys `config.xml` and
  `users.xml`, mounted with `subPath` as `config.d/nexora.xml` and `users.d/nexora.xml`: a ConfigMap
  key cannot contain `/`), StatefulSet and Service `clickhouse`.
- `deploy/kw/otelcol.yaml`: pipelines `logs/clickhouse` and `logs/loki`, and the writer password env.
- `deploy/kw/README.md`: the new manifest row, the `nexora-clickhouse` secret, the acceptance
  environment and a "Query-log backends" section.
...
- `scripts/kw-acceptance.sh`: the ClickHouse reader password and M10 environment, plus
  `TestKwQueryLogBackends` in the default run pattern and its package in the `go test` package list.
- `deploy/deploytest/kw_querylog_test.go`: created. `TestKwClickHouseManifest` and
  `TestKwCollectorFansOutQueryLogs`.
- `mgmt/internal/querylog/e2e/kw_querylog_backends_test.go`: created. `TestKwQueryLogBackends`.
  (The first draft put it in `e2e/`, whose import path cannot import the internal `querylog`
  package; the conformance e2e tests live in `mgmt/internal/querylog/e2e` for the same reason.)
...
- [x] Create `deploy/deploytest/kw_querylog_test.go`:
  - **`TestKwClickHouseManifest`:**
    ...
    - The StatefulSet `clickhouse` has:
      ...
      - ports 8123 and 9000.
    - The ConfigMap key `users.xml` (`users.d/nexora.xml`) contains `from_env="CLICKHOUSE_WRITER_PASSWORD"` and
      `from_env="CLICKHOUSE_READER_PASSWORD"`, `<ip>::1</ip>` and `<ip>127.0.0.1</ip>` under
      `default`, and no `<password>` element with text content. The key `config.xml` listens on
      `0.0.0.0`.
  ...
  - **`TestKwCollectorFansOutQueryLogs`:**
    ...
    - Assert that (every pipeline with receivers `[otlp]`):
    ...
    - When `exec.LookPath("otelcol-contrib")` succeeds (the dev pod), write the config to a temp file
      and require `otelcol-contrib validate --config` to exit 0 (with a dummy
      `CLICKHOUSE_WRITER_PASSWORD` in its environment). The test runs in the dev pod, where
...
- [x] Create `deploy/kw/clickhouse.yaml`:
  - **ConfigMap `clickhouse-config`:**
    - key `config.xml` (`config.d/nexora.xml`): `listen_host 0.0.0.0`, logger to console at level `warning`,
      `max_server_memory_usage_to_ram_ratio 0.8`.
    - key `users.xml` (`users.d/nexora.xml`):
      ...
      - `nexora_reader` with its `from_env` password and
        `<grants><query>GRANT SELECT ON nexora.querylog</query></grants>`, plus profile
        `nexora_reader` holding `<readonly>2</readonly>` (typed query parameters need settings
        changes; a `readonly` in profile `default` would also bind the writer and `default`).
...
- [x] Run the step-2 command and expect PASS.
- [x] Edit `scripts/kw-deploy.sh`, before the existing `k apply -f "$kw/opensearch.yaml" ...` line (the
      passwords go through files in the script's temporary directory, never a command line):
  ```bash
...
- [x] Create `mgmt/internal/querylog/e2e/kw_querylog_backends_test.go` with `TestKwQueryLogBackends`:
  1. Skip when `NEXORA_KW_DNS_ADDR` is unset (as `loadKwEnv` does, so `make e2e` passes), then
     `t.Fatalf` for a missing `NEXORA_KW_DNS_ADDR_2`, `NEXORA_KW_OPENSEARCH_URL`,
     `NEXORA_KW_CLICKHOUSE_URL`, `NEXORA_KW_CLICKHOUSE_PASSWORD_FILE` or `NEXORA_KW_LOKI_URL`.
...
- [x] Edit `scripts/kw-acceptance.sh`:
  ...
  - The default run pattern gains `|TestKwQueryLogBackends`, the header comment names it, and
    `go test` runs `-p 1 ... ./e2e/ ./mgmt/internal/querylog/e2e/` (one package at a time, so the
    query-log test does not overlap the smoke test's temporary configuration changes).
- [x] Edit `deploy/kw/README.md`:
  - Add a `clickhouse.yaml` row to the manifest table and the `nexora-clickhouse` secret.
  - Add a section "Query-log backends" after "Filter categories" (the collector paragraph there is
    followed by the filter categories' manual checks): logs fan out to OpenSearch (read by the console),
...
- [x] Run `scripts/dev-exec.sh 'go test ./deploy/deploytest -count=1 && go vet ./e2e/... ./mgmt/internal/querylog/e2e/'`
      and expect PASS.
...
- [ ] Edit `docs/operations.md`.
  ...
  - Add a **Loki (`loki`)** bullet:
    ...
    - Top uses two instant queries and reruns both over 37 partitions (names by first character after an
      optional `www.`, clients and categories by last character) when `max_query_series` is reached,
      and a partition still over the limit shows top lists as unavailable (raise `max_query_series`);
