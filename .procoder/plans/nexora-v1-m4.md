...
- Token keys whose generating transaction rolled back: `SweepTokenOrphans(ctx, st, box, grace)` lists token objects labelled `nexora-dnssec:<installation id>` (`Box.PKCS11SigningKeyRefs`; as changed in the pre-release sweep: migration `00502_installation.sql`, `store.InstallationID`, `secrets.Config.Installation`, so installations sharing a token never destroy each other's keys — `TestSweepLeavesOtherInstallationsTokenKeys`), records unreferenced ones in `dnssec_token_orphans` and destroys them once unreferenced for `OrphanGrace` (1 h); the maintainer sweeps every 10 min. `TestSweepDestroysTokenKeysOfRolledBackTransactions`.
...
- `deploy/docker/mgmt.Dockerfile` — (plan-review correction) build with `CGO_ENABLED=1` on `golang:1.27-trixie` and run on `debian:trixie-slim` (glibc, `ca-certificates`, user and group 65532) instead of `distroless/static`, so `NEXORA_PKCS11_MODULE` can load an HSM module; no module is shipped (kw keeps the KEK backend; README documents mounting one) (modify)
- `deploy/kw/mgmt.yaml` — also `runAsGroup: 65532`, so the gid matches `fsGroup` and the 0440 `nexora-kek` file passes the key-file permission check (modify)
- `deploy/kw/bind-primary.yaml` — in-cluster BIND primary `nexora-bind` (ClusterIP 10.43.200.53:5353, zone `bind-demo.kw.`, transfers only with TSIG key `nexora-demo-xfr.`) (create)
- `scripts/kw-deploy.sh` — also creates Secret `nexora-demo-tsig` (`name`, `algorithm`, `secret`, `named.key`) once and applies/rolls out `bind-primary.yaml` (modify)
- `deploy/kw/bootstrap.sh` — idempotently registers TSIG key `nexora-demo-xfr.`, primary zone `nexora-demo.kw.` (records, transfers and updates only with the key, transfers from 10.42.0.0/16, DNSSEC with `key_backend: kek`) and secondary zone `bind-demo.kw.` from `nexora-bind` (modify)
...
- [ ] Manual checks from the dev pod (commands in `deploy/kw/README.md`, "Authoritative demo zones"): `dig @192.168.10.136 nexora-demo.kw. SOA +dnssec` has `aa` and an RRSIG; `delv -a <anchor from GET /zones/{id}/dnssec> +root=nexora-demo.kw.` says `; fully validated`; AXFR with `-y hmac-sha256:nexora-demo-xfr.:<secret>` succeeds and without it fails; a signed `nsupdate` is applied (visible in DNS and `GET /zones/{id}/records`) and an unsigned one is REFUSED; `www.bind-demo.kw.` is answered with `aa`. The image's `nexora-mgmt` is linked against glibc (`ldd`) and passes `TestTSIGKeysWithPKCS11OnlyKeyStorage` with SoftHSM2 in the dev pod (`NEXORA_E2E_BIN_DIR` pointing at the binary copied from the image).
- [ ] Commit: `git add deploy/kw deploy/docker/mgmt.Dockerfile scripts/kw-deploy.sh e2e/kw_smoke_m4_test.go && git commit -m "deploy(kw): cgo mgmt image, demo zones and M4 smoke subtests"`.
