...
- Certificate issuance rate limit ('once per engine per 10 s') is per stream; reconnects bypass it. Make it per engine (e.g. last_issued_at on engine_certificates checked under row lock) with a test before release.
