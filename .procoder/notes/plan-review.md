...
- RESOLVED (verification sweep 2026-09-15: the fake OpenAI fixture gained held responses, `OpenAIResponse.Hold` released by `POST /control/release/{feature}`, test `TestOpenAIFixtureHoldsUntilReleased`; `gui_seed_ai_assistant_test.go` holds the config_assistant answer and `54-ai-assistant` asserts "Thinking" and no assistant message, then releases and asserts the reply and that the status clears): Flaky (M11 final check, 2026-09-15): `54-ai-assistant` can time out waiting for the "Thinking" status (the fake model may answer before the status renders). Fix in M11 Task 32 without weakening: assert the task reaches running or succeeded, or have the fixture hold the response until the spec observes the status.
...
- RESOLVED (verification sweep 2026-09-15): `51-ai-insights` flaked (1 in 3 runs) because `if (await ack.count())` ran before the findings list loaded, skipping the acknowledge step; the spec now waits for the anomaly that always stays open (`NEXORA_E2E_AI_ANOMALY_OPEN`) before deciding.
- RESOLVED (verification sweep 2026-09-15, found by the M10 agent): `TestQueryLogBackends/*/partial-name` sorted every `tube.test` name in the shared OpenSearch index and expected this run's name last; the case-insensitive search is now `YOU-<n>.TUBE.TEST` scoped to this run and expects exactly its name, and the run suffix includes seconds (unique across runs sharing an index).
- RESOLVED (verification sweep 2026-09-15): `TestQueryLogCategoryAttribution/opensearch` failed with an empty `list_name` for `www.custom.attr.test.`: the decision-reason names are fixed, and the shared OpenSearch index holds records of earlier runs whose lists no longer exist, so the lookup could return an old record. The reason lookups now pass `from=<subtest start>`.
- RESOLVED (verification sweep 2026-09-15): `TestNoAgentWritesConfiguration` failed with "no insight finding": the inserted hourly engine samples ended 30 minutes ago, outside the insight agent's 15-minute window, and interleaved with the engine's live samples (whose lower query counter counts as a restart), so a SERVFAIL spike only showed by chance. The history now ends one second before the engine's first live sample (checked to be under 10 minutes old), its newest pair two minutes apart, and the test asserts the `servfail_spike:<node>` insight specifically. Passed `-count=3`.
- OPEN (infrastructure, 2026-09-15): the amd64 ARC scale sets on the `novanas` cluster (`arc-azrtydxb-amd64`, `arc-azrtydxb-amd64-publish`) do not pick up jobs: both listeners in `arc-systems` are in `Error` and `arc-gha-rs-controller` logs `dial tcp 10.43.0.1:443: connect: no route to host`. images.yml amd64 builds (run 34946571726 queued since 08:22 UTC, holding the `images-refs/heads/main` concurrency group, so 34957578223 is pending) and perf-gate's `relative` job (dispatched run 34958192321) cannot run until that cluster's ARC controller reaches its API server. Not repo code; M7 Task 22 stays open on it.
- M8: rotateOdohKeyScheduled audit row (fixed identifier) has no task step; assign to Task 16 when briefing it.
