...
- Do NOT commit. Concurrent commits from stale indexes reverted other agents'
  work before. When your task is verified, list every path you created,
  modified or deleted in your final report; the lead commits serially.
  Never run `git add`, `git commit`, `git reset`, `git checkout --` or
  `git stash`.
